...
export default function AdminPage() {
  ...
  const [publisherDashboards, setPublisherDashboards] = useState<
    PublisherDashboard[]
  >([]);
  ...
  const [additionalData, setAdditionalData] = useState<{
    [key: number]: { genre?: string; nationality?: string; type?: string };
  }>({});
  const [activeTab, setActiveTab] = useState<
    "registrations" | "reviews" | "analytics" | "publishers"
  >("registrations");
          ...
          <>
            <div className="flex items-center justify-between mb-2">
              <h2 className="text-xl font-semibold">
                대기 중인 등록{" "}
                <span style={{ color: "#80FD8F" }}>
                  {registrations.length}개
                </span>
              </h2>
            </div>
                ...
                <div className="mt-8">
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() =>
                          handleApproval(registrations[0]?.id, "approve")
                        }
                      ...
                      <button
                        onClick={() =>
                          handleApproval(registrations[0]?.id, "reject")
                        }
                  ...
                  <div className="space-y-4">
                    {registrations.map((reg) => (
                      <div
                        key={reg.id}
                        className="p-4 border border-gray-700 rounded text-sm"
                      >
                            ...
                            <>
                              ...
                              <div>
                                ...
                                <a
                                  ...
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  className="text-point underline break-all"
                                >
                                  {reg.link}
                                </a>
                              </div>
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <input
                                  type="text"
                                  value={
                                    additionalData[reg.id]?.nationality || ""
                                  }
                                  onChange={(e) =>
                                    handleAdditionalDataChange(
                                      reg.id,
        ...
        {activeTab === "reviews" && (
          <div>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-semibold">
                서평단 관리{" "}
                <span style={{ color: "#80FD8F" }}>{reviews.length}개</span>
              </h2>
            </div>
            ...
            {reviewsLoading ? (
              ...
              <div className="space-y-6">
                          ...
                          >
                            <div className="flex justify-between items-start mb-2">
                              ...
                              <div className="flex gap-2">
                                ...
                                <button
                                  onClick={() =>
                                    copyPublisherLink(dashboard.token)
                                  }
                                  className="px-3 py-1 text-xs font-medium rounded transition-colors"
                                  style={{
                                    backgroundColor: "#80FD8F",
                ...
                {reviews.filter(
                  (review) =>
                    !publisherDashboards.find((d) => d.review_id === review.id)
                ).length > 0 && (
                  <div>
                    <h3 className="text-lg font-semibold mb-3 text-gray-300">
                      일반 서평단 (
                      {
                        reviews.filter(
                          (review) =>
                            !publisherDashboards.find(
                              (d) => d.review_id === review.id
                            )
                        ).length
                      }
                      개)
                          ...
                          >
                            <div className="flex justify-between items-start mb-2">
                              ...
                              <div className="flex gap-2">
                                ...
                                <button
                                  onClick={() =>
                                    createPublisherDashboard(review.id)
                                  }
        ...
        {activeTab === "analytics" && (
          <div>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-semibold">
                클릭 분석{" "}
                <span style={{ color: "#80FD8F" }}>{analytics.length}개</span>
              </h2>
              <div className="flex gap-2">
                <button
                    ...
                    >
                      <div className="flex justify-between items-start mb-2">
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-gray-500 mt-1">
                            마감: {formatDeadline(item.deadline)} | 등록일:{" "}
